...
```bash
cd server
docker compose up -d        # 启动 postgres:16-alpine，db=article_app，user/pass=app，端口 15432
```
...
APP_BASE_URL=http://localhost:9800  # 上传图片返回的绝对 URL 前缀
CORS_ORIGIN=http://localhost:7456    # 后台地址
```
...
看到 `Nest application successfully started` 即成功，监听 http://localhost:9800 。首次启动会自动建表（6 张：users/admins/categories/tags/articles/article_tags）。

快速验证：

```bash
curl -s -X POST http://localhost:9800/api/admin/auth/login \
...
```bash
cd server
pnpm exec ts-node scripts/seed-admin.ts admin pass123
...
```bash
cp admin/.env.example admin/.env    # 内容：VITE_API_BASE=http://localhost:9800/api
cd admin
pnpm run dev
```

打开 http://localhost:7456 ，用 **admin / pass123** 登录。
...
> 小程序请求地址在 `miniprogram/utils/request.ts` 的 `BASE`（开发为 `http://localhost:9800/api`）。**真机预览**时需改为电脑局域网 IP（如 `http://192.168.x.x:9800/api`），并在开发者工具同样关闭域名校验。
...
```bash
cd server
pnpm run test:e2e       # 已默认 --runInBand（串行，避免并发操作 DB）
```
...
**小程序首页"网络错误"**
- 后端没启动 → `cd server && pnpm run start:dev`。
...
**端口被占用**
- 后端 9800 / 后台 7456 / PG 15432 被占时，改对应配置（后端 `PORT` env、后台 vite 端口、docker-compose 端口）。

**wangEditor 插入的图片预览不了**
- 图片 URL 是 `APP_BASE_URL/uploads/xxx`，确保 `APP_BASE_URL` 指向后端地址；后台（:7456）跨域访问后端（:9800）已通过 CORS 放行。
