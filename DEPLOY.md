...
2. `cd server && pnpm exec ts-node scripts/seed-admin.ts <username> <password>` 建首个管理员。
...
cd server && pnpm run start:dev      # :9800
cd admin && pnpm run dev             # :7456
# 微信开发者工具打开 miniprogram/，关闭"不校验合法域名"
cd server && pnpm run test:e2e       # 32 个 e2e
```
